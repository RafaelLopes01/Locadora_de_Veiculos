...
void subMenu (){
    printf(" 0 SAIR  | 1 VEÍCULOS  | 2 CLIENTES  | 3 LOCACÕES  | 4 USUÁRIOS  | 5 RELATÓRIOS\n");
    linhaEstrela();
}
        ...
        do{
            ...
            switch(opcao){
                ...
                case 1 :
                    ...
                    if (op != 1){
                        do{
                            printf("\n COMBUSTÍVEL: 1 - Gasolina | 2 - Etanol | 3 - Flex: ");
                            scanf("%d", &op);
                            fflush(stdin);
                            switch (op) {
                                case 1:
            ...
            case 1 :
                    do{
                        ...
                        if ((strlen(nomeCli) ==  0)||(strlen(cpfCnpjCli) == 0)||(strlen(telefoneCli) == 0)||(strlen(enderecoCli) == 0)||(strlen(bairroCli) == 0)||(strlen(cidadeCli) == 0)||(strlen(cepCli) == 0)||(strlen(ufCli) == 0)){
                            printf("\n Todos o campos são de preenchimento Obrigatório!\n\n\n");
                            printf(" ");
                            op = -1;
                           // printf(" ");
                            //system("pause");
        ...
        switch(opcao){
            ...
            case 1 :
                do{
                    ...
                    linha();
                    printf("\n Entre com o CPF ou CNPJ do cliente(somente números): ");
                    scanf("%s", cpfCnpjCli);
                    pesqCli = inicioCli;
                    dif = -11;
                    contador = 0;
                        ...
                        break;
                           ...
                           // printf(" FAIXA ETÁRIA: %s", pesqCli->faixaEtaria);
                            ...
                            if (op == 1){
                                ...
                                printf("\n Data Locação: %s %s", locacao->dataIni, locacao->horaIni);
                                printf("\n Data devolução: %s %s", locacao->dataFim, locacao->horaIni);
            ...
            case 2 :
                ...
                if (auxLoc != NULL){
                    while ((auxLoc != NULL) && (strcmp(auxLoc->status, "Cancelado"))!= 0){
                        ...
                        printf("\n Data Locação: %s %s | ", auxLoc->dataIni, auxLoc->horaIni);
                        printf(" Data devolução: %s %s", auxLoc->dataFim, auxLoc->horaIni);
                        printf("\n Status: %s\n", auxLoc->status);
                ...
                if (auxLoc != NULL){
                    ...
                    while (auxLoc != NULL || dif == 0){
                        ...
                        if (dif == 0){
                            ...
                            printf("\n\n DIGITE: 0 - SAIR | 1 - CANCELAR LOCAÇÃO | 2 - FINALIZAR LOCAÇÃO: ");
                            scanf("%d", &op);
                            fflush(stdin);
                            switch (op){
                                case 0 :
                                ...
                                case 2 :
                                    ...
                                    printf("\n Se sim informar a quantidade de horas, se não informe ""0""(Zero): ");
                                    scanf("%d", &qtdHoras);
                                    strcpy(auxVei->status, "Livre");
        ...
        switch(opcao){
            ...
            case 1 :
                    do{
                        ...
                        senha[strlen(senha)-1]='\0';
                        fflush(stdin);
                        if ((strlen(nome) ==  0)||(strlen(user) == 0)||(strlen(senha) == 0)){ //Verifica se todos os campos estão preenchidos
                            printf("\n Todos o campos são de preenchimento Obrigatório!\n\n");
                            system("pause"); // Gera uma pausa até que alguma tecla seja pressionada
                        }
                    }while ((strlen(nome) ==  0)||(strlen(user) == 0)||(strlen(senha) == 0));//Realiza um loop até que todos os campos estejam preenchidos
                    dif = -1;
                    Usuario *check = inicioUser; //Cria uma struct auxiliar de nome check
                    while (check != NULL){ //Executa o loop até enquanto check for diferente de NULL
                        dif = strcmp(check->nomeUsuario, user); //compara se o usuário ja existe no cadastro
                        if (dif == 0){
                            printf("\n Usuário já cadastrado!\n");
                            printf("\n\n Cadastro cancelado!\n\n");
                            break;
                        }else{
                            check = check->proximoUser; //Enquanto check for diferente de NULL, vai para o próximo registro
                        }
                    }
                    ...
                    if (dif != 0){
                        ...
                        scanf("%d", &op);
                        if (op == 1){
                            Usuario *usuario = (Usuario*) malloc(sizeof(Usuario)); //Aloca um espaço de memória para struct usuário
                            strcpy(usuario->nomeCompleto, nome); // copia o conteúdo da variável para a struct
                            strcpy(usuario->nomeUsuario, user);
                            strcpy(usuario->senha, senha);
                            usuario->proximoUser = NULL;
                            if (inicioUser == NULL){ //Se não houver nenhum cadastro
                                inicioUser = usuario;// Cadastra o primeiro registro
                            }else{ //Se não
                                usuario->proximoUser = inicioUser; //Aponta para o próximo registro
        ...
        switch (op){
            ...
            case 3 :
                ...
                while (aux != NULL){
                    ...
                    printf("\n DEVOLUÇÃO: %s %s", aux->dataFim, aux->horaFim);
                    printf("\n STATUS: %s", aux->status);
                    printf("\n");
                    linha();
                    aux = aux->proximoLoc;
